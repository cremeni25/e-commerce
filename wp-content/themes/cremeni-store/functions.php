<?php
/**
 * Configuração principal do tema CREMENI.
 *
...
function cremeni_store_setup(): void
{
    ...
    register_nav_menus([
        ...
        'categories' => __('Menu de universos', 'cremeni-store'),
...
function cremeni_store_layout_guard(): void
{
    ...
    <style id="cremeni-layout-guard">
        html,body{width:100%!important;min-width:100%!important;max-width:none!important;margin:0!important;padding:0!important}
        body{display:block!important;background:#0d0d0d!important;overflow-x:hidden!important;transform:none!important;zoom:1!important}
        .site-header,#conteudo,.site-footer{width:100%!important;max-width:none!important;margin-inline:0!important}
        .cremeni-container{width:min(calc(100% - 48px),1440px)!important;max-width:1440px!important;min-width:0!important;margin-left:auto!important;margin-right:auto!important}
        .hero__grid{grid-template-columns:minmax(0,1.08fr) minmax(360px,.92fr)!important}
        .hero h1{font-size:clamp(3.6rem,5.4vw,6.2rem)!important}
        @media(max-width:900px){.cremeni-container{width:min(calc(100% - 32px),100%)!important}.hero__grid{grid-template-columns:1fr!important}.hero h1{font-size:clamp(2.8rem,11vw,5rem)!important}}
    </style>
    <?php
}
...
/** Três universos comerciais oficiais da CREMENI. */
function cremeni_store_product_categories(): array
{
    return [
        'esporte' => [
            'label' => __('CREMENI Esporte', 'cremeni-store'),
            'description' => __('Produtos selecionados para movimento, treino, prática esportiva e performance.', 'cremeni-store'),
        ],
        'pet-mimos' => [
        ...
        'guias-cremeni' => [
            ...
            'description' => __('Conteúdo próprio para corpo, mente, rotina, convivência e vida ativa.', 'cremeni-store'),
        ],
    ];
}
...
function cremeni_store_woocommerce_loop_columns(): int { return 4; }
add_filter('loop_shop_columns', 'cremeni_store_woocommerce_loop_columns');
function cremeni_store_woocommerce_products_per_page(): int { return 12; }
