...
<section id="esportes" class="sports-section"><div class="cremeni-container"><div class="section-heading"><p class="eyebrow"><?php esc_html_e('CREMENI ESPORTE', 'cremeni-store'); ?></p><h2><?php esc_html_e('Seu esporte. Sua rotina.', 'cremeni-store'); ?></h2><p><?php esc_html_e('Uma seleção objetiva por modalidade, sem catálogo inflado e sem produto sem função.', 'cremeni-store'); ?></p></div><div class="sports-grid"><?php foreach($sports as $slug=>$sport): ?><a class="sport-card" href="<?php echo esc_url(add_query_arg('esporte',$slug,$shop_url)); ?>"><span class="sport-card__mark"><?php echo esc_html(cremeni_store_initial($sport)); ?></span><strong><?php echo esc_html($sport); ?></strong></a><?php endforeach; ?></div></div></section>
