 ...
 * Description: Materializa o primeiro catálogo real CREMENI Esporte, visível e ainda não comprável até ativação operacional.
 * Version: 1.0.0
 * Author: CREMENI
 */
...
const CREMENI_CATALOG_PILOT_VERSION = '1.0.0';
...
function cremeni_catalog_pilot_products(): array {
    return [
        'cremeni-raquete-beach-tennis-personalizada' => [
            ...
            'short' => 'Raquete personalizável para prática de Beach Tennis, selecionada para o catálogo inicial CREMENI Esporte.',
            'description' => '<h2>Beach Tennis com identidade própria.</h2><p>Modelo personalizável com nome, marca, logotipo, foto, arte ou frase. A seleção CREMENI prioriza produtos com função esportiva clara, personalização e operação nacional.</p><h3>Antes da compra</h3><p>A personalização e o prazo de produção serão confirmados antes da liberação comercial definitiva.</p>',
            ...
            'facts' => ['Personalizável','Dropshipping nacional confirmado','Estoque validado em 09/09/2026'],
        ],
        'cremeni-colchonete-treino-yoga-personalizado' => [
            ...
            'facts' => ['Treino, Pilates e Yoga','Personalizável','Estoque validado em 09/09/2026'],
        ],
        'cremeni-kit-agilidade-8-cones-4-bastoes' => [
            ...
            'facts' => ['8 cones de 24 cm','4 bastões de 80 cm','Estoque validado em 09/09/2026'],
        ],
        'cremeni-prancha-aprendizagem-natacao-personalizada' => [
            ...
            'facts' => ['Apoio à aprendizagem','Personalizável','Estoque validado em 09/09/2026'],
        ],
    ];
}
...
function cremeni_catalog_pilot_not_purchasable(bool $purchasable, WC_Product $product): bool {
    if (get_post_meta($product->get_id(), '_cremeni_sales_enabled', true) === 'no') { return false; }
    return $purchasable;
}
...
function cremeni_catalog_pilot_card_status(): void {
    ...
    if (get_post_meta($product->get_id(), '_cremeni_commercial_status', true) === 'pre_operacional') {
...
function cremeni_catalog_pilot_single_status(): void {
    global $product;
    if (! $product instanceof WC_Product) { return; }
    if (get_post_meta($product->get_id(), '_cremeni_commercial_status', true) !== 'pre_operacional') { return; }

    $facts = json_decode((string) get_post_meta($product->get_id(), '_cremeni_public_facts', true), true);
    echo '<div class="cremeni-product-readiness"><strong>' . esc_html__('Produto real em preparação comercial', 'cremeni-store') . '</strong><p>' . esc_html__('Fornecedor, estoque e operação foram validados. A compra será liberada somente após ativação da condição operacional CREMENI.', 'cremeni-store') . '</p>';
    if (is_array($facts) && $facts !== []) {
        echo '<ul>';
        foreach ($facts as $fact) { echo '<li>' . esc_html((string) $fact) . '</li>'; }
        echo '</ul>';
    }
    echo '</div>';
}
