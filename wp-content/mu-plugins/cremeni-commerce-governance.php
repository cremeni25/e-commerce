...
const CREMENI_GOVERNANCE_VERSION = '0.1.0';
...
function cremeni_governance_meta_keys(): array
{
    return [
        '_cremeni_governance_enabled',
        '_cremeni_vertical',
        '_cremeni_commercial_role',
        '_cremeni_supplier',
        '_cremeni_supplier_sku',
        '_cremeni_drop_confirmed',
        '_cremeni_drop_cost',
        '_cremeni_market_reference',
        '_cremeni_target_price',
        '_cremeni_incremental_costs',
        '_cremeni_incremental_freight',
        '_cremeni_supplier_stock',
        '_cremeni_cost_valid_until',
        '_cremeni_drop_valid_until',
        '_cremeni_personalization',
        '_cremeni_related_guide',
        '_cremeni_commercial_status',
        '_cremeni_last_evaluation',
    ];
}
...
function cremeni_governance_evaluate(int $product_id): array
{
    ...
    if (cremeni_governance_date_is_expired($cost_valid_until)) {
        $blockers[] = 'Validade do custo drop expirou.';
    }

    if (cremeni_governance_date_is_expired($drop_valid_until)) {
    ...
    if ($vertical === 'pet_premium' && $drop_cost > 0 && $target_price > 0 && $incremental_contribution <= 0) {
        $blockers[] = 'PET Premium sem contribuição incremental positiva.';
    }

    if ($vertical === 'guias' && $target_price < 0) {
        $blockers[] = 'Preço do Guia Cremeni inválido.';
    }
...
function cremeni_governance_render_meta_box(WP_Post $post): void
{
    ...
    ?>
    <p>
        <label><input type="checkbox" name="cremeni_governance_enabled" value="1" <?php checked($get('_cremeni_governance_enabled'), '1'); ?>> Ativar governança comercial para este SKU</label>
    </p>
...
add_action('save_post_product', 'cremeni_governance_save_product', 20);
